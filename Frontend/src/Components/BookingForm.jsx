...
const BookingForm = () => {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      ...
      setError(null);
      try {
        const res = await getAvailableRooms(token);
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await createBooking({ roomId: selectedRoom._id, ...formData }, token);
      ...
      {!selectedRoom ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rooms.map((room) => (
            ...
            >
              <img src={roomImage} alt={room.roomNumber} className="w-full h-40 object-cover rounded-lg mb-4" />
              <h3 className="text-xl font-bold text-gray-800">{room.roomNumber} - {room.type}</h3>
              <p className="text-gray-600">Price: ${room.currentPrice.toFixed(2)} / night</p>
        ...
        <div className="max-w-lg mx-auto p-6 bg-white rounded-xl shadow-lg">
          ...
          <img src={roomImage} alt={selectedRoom.roomNumber} className="w-full h-48 object-cover rounded-lg mb-6" />
